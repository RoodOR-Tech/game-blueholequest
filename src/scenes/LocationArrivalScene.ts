...
export class LocationArrivalScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    const location = BOSS_LOCATIONS[this.routeIndex] ?? bossLocationById('hillsboro_west');
    const arrival = LOCATION_ARRIVALS[location.id];
    this.drawBackdrop(location.id);
    this.add.rectangle(128, 43, 236, 57, 0x07111c, 0.88).setStrokeStyle(2, location.color);
    this.add.text(128, 27, arrival.progress, {
      color: '#9ec5d8', fontFamily: 'monospace', fontSize: '6px', fontStyle: 'bold',
    }).setOrigin(0.5);
    this.add.text(128, 45, arrival.title, {
      align: 'center', color: '#f6d77a', fontFamily: 'monospace', fontSize: '11px', fontStyle: 'bold',
    }).setOrigin(0.5);
  ...
  private drawBackdrop(locationId: keyof typeof LOCATION_ARRIVALS): void {
    ...
    if (locationId === 'hillsboro_west') {
      g.fillGradientStyle(0x09101d, 0x09101d, 0x263549, 0x263549).fillRect(0, 0, 256, 240);
      g.fillStyle(0xd8e5cf, 0.75).fillCircle(202, 103, 25);
      for (let x = -4; x < 270; x += 25) g.fillStyle(x % 50 ? 0x14291f : 0x0c1c18).fillTriangle(x, 203, x + 14, 91 + (x % 3) * 9, x + 29, 203);
      g.fillStyle(0xb8cbd0, 0.18).fillEllipse(128, 177, 290, 42).fillEllipse(80, 197, 210, 30);
      g.fillStyle(0x090d10).fillCircle(167, 165, 5).fillRect(164, 169, 6, 20);
      this.add.text(172, 157, '?', { color: '#ff8c68', fontFamily: 'monospace', fontSize: '8px' });
    } else if (locationId === 'hillsboro_east') {
      g.fillGradientStyle(0x183252, 0x183252, 0xe18b5b, 0xe18b5b).fillRect(0, 0, 256, 240);
      for (let x = 0; x < 256; x += 34) g.fillStyle(0x172331).fillRect(x, 132 - (x % 4) * 8, 28, 75);
      g.lineStyle(3, 0x171b24).lineBetween(26, 103, 26, 210).lineBetween(224, 91, 224, 210).lineBetween(26, 111, 224, 99);
      g.lineStyle(1, 0x6ee6ff).lineBetween(27, 115, 223, 103);
    } else if (locationId === 'milwaukie') {
      g.fillGradientStyle(0x274358, 0x274358, 0x95b7b0, 0x95b7b0).fillRect(0, 0, 256, 240);
      ...
      g.lineStyle(2, 0xb9efff, 0.35).lineBetween(0, 178, 256, 170).lineBetween(0, 201, 256, 190);
    } else if (locationId === 'walla_walla') {
      g.fillGradientStyle(0x5e8ac2, 0x5e8ac2, 0xf0ba6d, 0xf0ba6d).fillRect(0, 0, 256, 240);
      ...
      g.fillGradientStyle(0x321e35, 0x321e35, 0xf0784c, 0xf0784c).fillRect(0, 0, 256, 240);
      g.fillStyle(0x343840).fillTriangle(42, 194, 111, 79, 174, 194).fillTriangle(111, 194, 181, 101, 237, 194);
