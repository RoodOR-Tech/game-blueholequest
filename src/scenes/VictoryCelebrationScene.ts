...
const FIREWORK_COLORS = [0xff5e75, 0x68d8ff, 0xf6d77a, 0x7de06f, 0xd997ff] as const;
...
export class VictoryCelebrationScene extends Phaser.Scene {
  ...
  private drawPartyGround(): void {
    ...
    g.fillGradientStyle(0x05091d, 0x05091d, 0x241747, 0x241747).fillRect(0, 0, 256, 240);
    g.fillStyle(0x0c2741).fillTriangle(0, 185, 54, 105, 108, 185).fillTriangle(65, 185, 137, 89, 199, 185).fillTriangle(138, 185, 216, 111, 270, 185);
    ...
    for (let x = 8; x < 256; x += 16)
      g.fillStyle(FIREWORK_COLORS[(x / 16) % FIREWORK_COLORS.length] ?? 0xffffff).fillCircle(x, 84 + (x % 32 ? 5 : 0), 2);
  }
  ...
  private createArtifactDisplay(): void {
    FIREWORK_COLORS.forEach((color, index) => {
      const x = 92 + index * 18;
      const relic = this.add.rectangle(x, 91, 9, 13, color).setStrokeStyle(2, 0xffffff, 0.65).setRotation(Math.PI / 4);
      this.tweens.add({ targets: relic, y: 86, scale: 1.15, duration: 500 + index * 90, yoyo: true, repeat: -1 });
    });
  }

  private createConfetti(): void {
    for (let i = 0; i < 32; i += 1) {
      const piece = this.add.rectangle(Phaser.Math.Between(5, 251), Phaser.Math.Between(72, 174), 2, 4, FIREWORK_COLORS[i % FIREWORK_COLORS.length] ?? 0xffffff).setRotation(Math.random() * Math.PI);
      this.tweens.add({ targets: piece, y: piece.y + Phaser.Math.Between(25, 65), angle: piece.angle + 220, alpha: 0.15, duration: Phaser.Math.Between(1200, 2400), yoyo: true, repeat: -1, delay: Phaser.Math.Between(0, 700) });
    }
  }
  ...
  private launchFirework(): void {
    ...
    for (let i = 0; i < 14; i += 1) {
      ...
      this.tweens.add({ targets: spark, x: x + Math.cos(angle) * Phaser.Math.Between(18, 35), y: y + Math.sin(angle) * Phaser.Math.Between(18, 35), alpha: 0, scale: 0.3, duration: 520, onComplete: () => spark.destroy() });
    }
